...
odoo.define('employee_attendance_geolocations.google_map_wizard', function (require) {
  "use strict";
  ...
  FormRenderer.include({
    _renderView: function () {
      ...
      return this._super.apply(this, arguments).then(function () {
        if (self.state.model == 'employee.attendance.location.wizard') {
          self._renderEmployeeLocationOnMap();
          if (self.state.data.active_cluster) {
            self.filter = {};
            self._renderFilter();
          }
        }
    ...
    _renderFilter: async function () {
      var self = this;
      await ajax.loadXML('employee_attendance_geolocations/static/src/xml/attendance_advance_filter.xml', core.qweb)
      this.$filter =  $(core.qweb.render('employee_attendance_geolocations_filter', {today: (new Date()).toISOString().split('T')[0]}))
    ...
    _renderEmployeeLocationOnMap: async function () {
      var self = this;
      this.$map = this.$el.find('#employee_location');
      this.markers = [];
      this.bounds = new google.maps.LatLngBounds();
      this.infoWindow = new google.maps.InfoWindow();

      var response = await this._rpc({
        route: '/hr_attendance_geolocation',
        params: {
          hr_attendance_ids: this.state.data.hr_attendance_ids.res_ids,
          filter: this.filter
        }
      });
      ...
      if (this.state.data.active_cluster) {
        if (this.$filter) {
